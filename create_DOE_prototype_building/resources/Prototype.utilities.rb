...
def safe_load_model(model_path_string)  
  ...
  if OpenStudio::exists(model_path)
    ...
    if model.empty?
      OpenStudio::logFree(OpenStudio::Error, "openstudio.prototype.Model", "Version translation failed for #{model_path_string}")
      return false
    else
      model = model.get
    end
  else
    OpenStudio::logFree(OpenStudio::Error, "openstudio.prototype.Model", "#{model_path_string} couldn't be found")
    return false
  end
  return model
end
...
def safe_load_sql(sql_path_string)
  ...
  if OpenStudio::exists(sql_path)
    ...
    OpenStudio::logFree(OpenStudio::Error, "openstudio.prototype.Model", "#{sql_path} couldn't be found")
    return false
  end
  return sql
end
...
def find_objects(hash_of_objects, search_criteria, capacity = nil)
  ...
  if matching_objects.size == 0
    desired_object = nil
    OpenStudio::logFree(OpenStudio::Warn, "openstudio.prototype.Model", "Find objects search criteria returned no results. Search criteria: #{search_criteria}, capacity = #{capacity}.  Called from #{caller(0)[1]}.")
  end
...
def find_object(hash_of_objects, search_criteria, capacity = nil)
  ...
  if matching_objects.size == 0
    desired_object = nil
    #OpenStudio::logFree(OpenStudio::Warn, "openstudio.prototype.Model", "Find object search criteria returned no results. Search criteria: #{search_criteria}, capacity = #{capacity}.  Called from #{caller(0)[1]}")
  elsif matching_objects.size == 1
    desired_object = matching_objects[0]
  else 
    desired_object = matching_objects[0]
    OpenStudio::logFree(OpenStudio::Warn, "openstudio.prototype.Model", "Find object search criteria returned #{matching_objects.size} results, the first one will be returned. Search criteria: #{search_criteria} Called from #{caller(0)[1]}.  All results: \n #{matching_objects.join("\n")}")
  end
