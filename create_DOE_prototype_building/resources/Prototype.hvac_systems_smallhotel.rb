...
class OpenStudio::Model::Model
  ...
  def add_split_AC(prototype_input, hvac_standards, thermal_zones)
    ...
    # Through: 12/31,
    # For: All days: always on: Until: 24:00, 1.0
    #all days
    hvac_op_sch = OpenStudio::Model::ScheduleRuleset.new(self)
    hvac_op_sch.setName("HVAC Operation Schedule")
    hvac_op_sch.defaultDaySchedule.setName("HVAC Operation Schedule day") 
    hvac_op_sch.defaultDaySchedule.addValue(OpenStudio::Time.new(0,24,0,0), 1.0)
    hvac_op_sch.setSummerDesignDaySchedule(hvac_op_sch.defaultDaySchedule)
    hvac_op_sch.setWinterDesignDaySchedule(hvac_op_sch.defaultDaySchedule)
    ...
    motorized_oa_damper_sch.defaultDaySchedule.setName("Motorized OA Damper Schedule day") 
    motorized_oa_damper_sch.defaultDaySchedule.addValue(OpenStudio::Time.new(0,24,0,0), 1.0)
    motorized_oa_damper_sch.setSummerDesignDaySchedule(motorized_oa_damper_sch.defaultDaySchedule)
    motorized_oa_damper_sch.setWinterDesignDaySchedule(motorized_oa_damper_sch.defaultDaySchedule)
  ...
  def add_psz_ac_smallhotel(prototype_input, hvac_standards, thermal_zones)
    ...
    thermal_zones.each do |zone|
      ...
      fan = nil
      if prototype_input["psz_ac_fan_type"] == "ConstantVolume"
        ...
        fan.setFanEfficiency(0.53625)
        fan.setMotorEfficiency(0.825)
      elsif prototype_input["psz_ac_fan_type"] == "Cycling" 
      ...
      htg_coil = nil
      if prototype_input["psz_ac_heating_type"] == "Gas"
        htg_coil = OpenStudio::Model::CoilHeatingGas.new(self,self.alwaysOnDiscreteSchedule)
        htg_coil.setName("#{zone.name} PSZ-AC Gas Htg Coil")
        htg_coil.setGasBurnerEfficiency(0.8)
      elsif prototype_input["psz_ac_heating_type"] == "Single Speed Heat Pump"
        htg_cap_f_of_temp = OpenStudio::Model::CurveCubic.new(self)
        htg_cap_f_of_temp.setCoefficient1Constant(0.758746)
        htg_cap_f_of_temp.setCoefficient2x(0.027626)
        htg_cap_f_of_temp.setCoefficient3xPOW2(0.000148716)
      ...
      clg_coil = nil
      if prototype_input["psz_ac_cooling_type"] == "Two Speed DX AC"
      ...
      elsif prototype_input["psz_ac_cooling_type"] == "Single Speed DX AC"
      ...
      elsif prototype_input["psz_ac_cooling_type"] == "Single Speed Heat Pump"
      ...
      # Wrap coils in a psz system or not, depending
      # on the system type.
      if prototype_input["psz_ac_fan_type"] == "Cycling"
  ...
  def add_swh_loop(prototype_input, hvac_standards)
    ...
    if template == '90.1-2010'
      ...
      swh_pump.setName('Service Water Loop Pump')
      swh_pump_head_press_pa = prototype_input['service_water_pump_head'] # As if there is no circulation pump
      swh_pump_motor_efficiency = prototype_input['service_water_pump_motor_efficiency']
      swh_pump.setRatedPumpHead(swh_pump_head_press_pa)
      swh_pump.setMotorEfficiency(swh_pump_motor_efficiency)
      swh_pump.setFractionofMotorInefficienciestoFluidStream(0)
    ...
    default_water_heater_ambient_temp_sch.defaultDaySchedule.addValue(OpenStudio::Time.new(0,24,0,0),OpenStudio::convert(71.6,"F","C").get)
    ...
    if water_heater_fuel == 'Electricity'
      ...
      if water_heater_vol_gal.between?(290,310)
        water_heater.setOffCycleParasiticFuelConsumptionRate(1568)
        water_heater.setOffCycleParasiticFuelType('NaturalGas')
        water_heater.setOnCycleParasiticFuelType('NaturalGas')
        water_heater.setOnCycleParasiticFuelConsumptionRate(1568)
        water_heater.setOffCycleParasiticHeatFractiontoTank(0.8)
        water_heater.setOffCycleLossCoefficienttoAmbientTemperature(11.25)
        water_heater.setOnCycleLossCoefficienttoAmbientTemperature(11.25)
      elsif water_heater_vol_gal.between?(190,210)
        water_heater.setOffCycleParasiticFuelType('NaturalGas')
        water_heater.setOnCycleParasiticFuelType('NaturalGas')
        water_heater.setOffCycleParasiticHeatFractiontoTank(0.8)
        water_heater.setOffCycleLossCoefficienttoAmbientTemperature(9.64)
        water_heater.setOnCycleLossCoefficienttoAmbientTemperature(9.64)
      else
        water_heater.setOffCycleParasiticFuelConsumptionRate(20)
        water_heater.setOffCycleParasiticFuelType('NaturalGas')
        water_heater.setOnCycleParasiticFuelType('NaturalGas')
        water_heater.setOnCycleParasiticFuelConsumptionRate(20)
