...
class CreateDOEPrototypeBuildingTest < Minitest::Unit::TestCase
  ...
  # Create a set of models, return a list of failures
  def compare_results(building_type, building_vintage, climate_zone)
    ...
    File.open("#{Dir.pwd}/build/#{model_name}/comparison.csv", 'w') do |file|
      ...
      results_hash.each_pair do |key1, value1|  # building type
        value1.each_pair do |key2, value2| # vintage
          value2.each_pair do |key3, value3| # climate zone
  ...
  def test_large_hotel
    building_type = ARGV[0]#['LargeHotel']
    building_vintage = ARGV[1]#['90.1-2010'] #, 'DOE Ref Pre-1980', 'DOE Ref 1980-2004']
    climate_zone = ARGV[2]#['ASHRAE 169-2006-2A']

    all_failures = []

    # Create the models
    #all_failures += create_models(building_type, building_vintage, climate_zone)

    # Run the models
    #all_failures += run_models(building_type, building_vintage, climate_zone)

    # Compare the results to the legacy idf results
    all_failures += compare_results(building_type, building_vintage, climate_zone)
